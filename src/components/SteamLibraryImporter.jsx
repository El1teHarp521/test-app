...
function SteamLibraryImporter({ onGamesImported, existingTechnologies = [] }) {
    ...
    const [steamId, setSteamId] = useState(STEAM_USER_ID);
    ...
    const loadUserProfile = async () => {
        try {
            const profile = await fetchUserProfile(steamId);
    ...
    const loadGames = async () => {
        try {
            const idToUse = customSteamId.trim() || steamId;
            ...
            // Сбрасываем выбранные игры при загрузке новых
            setSelectedGames([]);
        } catch (err) {
    ...
    const handleImportGames = async () => {
        ...
        try {
            ...
            // Передаем игры в родительский компонент
            if (onGamesImported) {
